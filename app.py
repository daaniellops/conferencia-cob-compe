...
st.title("📊 Conferência Financeira - Agrupada por Categoria")
st.subheader("Processador Automático de Créditos e Débitos (Sem Saldos)")
    ...
    with pdfplumber.open(pdf_bytes) as pdf:
        ...
        for page in pdf.pages:
            ...
            for linha in linhas:
                ...
                # IGNORA ESTRITAMENTE QUALQUER LINHA QUE SEJA SALDO
                if re.search(r'\bSALDO\b', linha_clean, re.IGNORECASE) or linha_clean.startswith('000000'):
                    continue
                
                # Procura por linhas com valores de movimentação terminadas em C ou D
...
def gerar_tabela_html(df_sub):
    ...
    resumo_totais = f"Créditos: {fmt_brl(total_c)} | Débitos: {fmt_brl(total_d)}"
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            <tr style="font-weight: bold; background-color: #f8fafc;">
                <td colspan="3" style="text-align: right;">TOTAIS DA CATEGORIA:</td>
...
def gerar_pdf_relatorio(meta, df_mov):
    ...
    <head>
        ...
        <style>
            ...
            .section-title {{ color: #1e3a8a; font-size: 10pt; margin-top: 10px; margin-bottom: 6px; border-bottom: 1px solid #cbd5e1; padding-bottom: 3px; }}
        </style>
    </head>
    <body>
        <div class="header">
            <h2 style="margin:0; font-size: 13pt;">Relatório de Conferência Financeira</h2>
            <p style="margin:2px 0 0 0; font-size: 8.5pt;">Agrupado por Tópicos (Exclusão de Saldos)</p>
        </div>
        ...
        <table class="cards-table">
            <tr>
                <td width="32%" class="card">
                    <div class="card-title">Total Entradas (Créditos)</div>
                    <div class="card-value" style="color: #15803d;">{fmt_brl(total_creditos)}</div>
                </td>
                <td width="2%"></td>
                <td width="32%" class="card">
                    <div class="card-title">Total Saídas (Débitos)</div>
        ...
        <div class="section-title">1. Lançamentos COB COMPE</div>
        {gerar_tabela_html(df_cob)}

        <div class="section-title">2. Lançamentos RESG AUT / APLICAÇÃO</div>
        ...
        <div style="margin-top: 10px; font-size: 7pt; color: #64748b;">
            <b>Observação de Conciliação:</b> Lançamentos de saldos foram desconsiderados dos cálculos para garantir a apuração das movimentações reais de fluxo de caixa.
        </div>
    </body>
    </html>
    """
...
if arquivo_pdf is not None:
    ...
    if not df_mov.empty:
        ...
        # Exibição na Tela do Streamlit Separada por Abas
        tab1, tab2, tab3 = st.tabs(["📌 COB COMPE", "🔄 RESG AUT / APLICAÇÃO", "📋 OUTRAS MOVIMENTAÇÕES"])
        
        with tab1:
            df_cob = df_mov[df_mov['Categoria'] == 'COB COMPE']
            st.dataframe(df_cob[['Data Mov.', 'Histórico', 'Tipo', 'Valor Formatado']], use_container_width=True)
            
        with tab2:
            df_resg = df_mov[df_mov['Categoria'] == 'RESG AUT / APLICAÇÃO']
            st.dataframe(df_resg[['Data Mov.', 'Histórico', 'Tipo', 'Valor Formatado']], use_container_width=True)
            
        with tab3:
            df_outros = df_mov[df_mov['Categoria'] == 'OUTRAS MOVIMENTAÇÕES']
            st.dataframe(df_outros[['Data Mov.', 'Histórico', 'Tipo', 'Valor Formatado']], use_container_width=True)
            
        pdf_out = gerar_pdf_relatorio(meta, df_mov)
        
        st.download_button(
            label="📄 Baixar Relatório Categorizado em PDF",
            data=pdf_out,
            file_name="Relatorio_Conferencia_Categorizada.pdf",
